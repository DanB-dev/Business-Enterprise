...
public interface UserRepository extends CrudRepository<User, String>, JpaSpecificationExecutor<User> {
	...
	void updateDetailsWithoutPassword(@Param("username") String username, @Param("admin") Boolean admin,
			@Param("power") Boolean power, @Param("user") Boolean user, @Param("id") String id);

	@Transactional
	@Modifying
	@Query("update User u set u.username= :username, u.admin = :admin, u.power= :power, u.user= :user,u.password= :password where u.id = :id")
	void updateDetailsWithPassword(@Param("username") String username, @Param("admin") Boolean admin,
			@Param("power") Boolean power, @Param("user") Boolean user, @Param("password") String password,
			@Param("id") String id);
